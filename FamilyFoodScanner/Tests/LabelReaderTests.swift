...
final class LabelReaderTests: XCTestCase {
    ...
    func testReadsIngredientsAndNutritionFromAPhotoOfALabel() async throws {
        ...
        guard case .label(let parsed, _, _) = outcome else { return XCTFail("expected a label, got \(outcome)") }
    ...
    func testAPhotoWithNothingUsefulIsReportedHonestly() async throws {
        ...
        guard case .label(let parsed, _, _) = try await LabelReader.read([blank]) else { return XCTFail("expected a label") }
